...
static void gpio_keyboard_scan(struct work_struct *work)
{
	...
	u32 new_state[MATRIX_MAX_COLS] = { 0 };
	...
	for (col = 0; col < gkb->col_gpios->ndescs; col++) {
		...
		bits_changed = gkb->last_key_state[col] ^ new_state[col];
		if (bits_changed == 0)
			continue;
	...
	if (gkb->autorescan_ms && keymask) {
		schedule_delayed_work(&gkb->work,
				      msecs_to_jiffies(gkb->autorescan_ms));
		return;
	}
end:
	/* Enable IRQs again */
	spin_lock_irq(&gkb->lock);
	gkb->scan_pending = false;
	enable_row_irqs(gkb);
	spin_unlock_irq(&gkb->lock);
}
...
static irqreturn_t gpio_keyboard_interrupt(int irq, void *id)
{
	...
	spin_lock_irqsave(&gkb->lock, flags);
	...
	mod_delayed_work(system_wq, &gkb->work,
		msecs_to_jiffies(gkb->debounce_ms));

out:
	spin_unlock_irqrestore(&gkb->lock, flags);
	return IRQ_HANDLED;
}
