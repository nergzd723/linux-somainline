 ...
 * @hwp: pin bit number in port register
 *
 ...
 * @regmap: Regmap structure
 * @pin: gpiolib pin number
 * @hw_pin: hw pin bit number, used to retrieve port number
				   ...
				   u8 hw_pin, unsigned int *state)
{
	...
	ret = regmap_read(regmap, reg, state);
	if (ret)
		return ret;
...
static irqreturn_t aw9523_irq_thread_func(int irq, void *dev_id)
{
	...
	if (regmap_read(awi->regmap, AW9523_REG_IN_STATE(0), &tmp))
		return IRQ_NONE;
	val = (u8)tmp;

	if (regmap_read(awi->regmap,
			AW9523_REG_IN_STATE(AW9523_PINS_PER_PORT), &tmp))
		return IRQ_NONE;
	val |= (u8)tmp << 8;
	...
	/*
	 * To avoid *slow* i2c reads on four more registers, just
	 * check for irq_find_mapping result: if the interrupt is
	 * not mapped, then we don't want to care about it.
	 */
	for_each_set_bit(n, &changed_gpio, awi->gpio.ngpio) {
		tmp = irq_find_mapping(awi->gpio.irq.domain, n);
		if (tmp <= 0)
			continue;
		handle_nested_irq(irq_find_mapping(awi->gpio.irq.domain, n));
	}
				    ...
				    unsigned long *bits)
{
	...
	/* Port 1 (gpio 8-15) */
	if (*mask & (U8_MAX << 8)) {
		ret = __aw9523_get_port_state(awi->regmap,
					      AW9523_PINS_PER_PORT, 0, &p1);
		if (ret)
			goto out;
	}

	/* Port 0 (gpio 0-7) */
	ret = __aw9523_get_port_state(awi->regmap, 0, 0, &p0);
	if (ret)
		goto out;
...
static int aw9523_init_gpiochip(struct aw9523 *awi, struct pinctrl_desc *pdesc)
{
...
static int aw9523_hw_init(struct aw9523 *awi)
{
	...
	ret = regmap_write(awi->regmap, AW9523_REG_PORT_MODE(0), 0xff);
	if (ret)
		return ret;
	ret = regmap_write(awi->regmap, AW9523_REG_PORT_MODE(p1_pin), 0xff);
	if (ret)
		return ret;
	...
	ret = regmap_write(awi->regmap, AW9523_REG_CONF_STATE(0), 0xff);
	if (ret)
		return ret;
	ret = regmap_write(awi->regmap, AW9523_REG_CONF_STATE(p1_pin), 0xff);
	if (ret)
		return ret;

	/* Disable all interrupts to avoid unreasoned wakeups */
	ret = regmap_write(awi->regmap, AW9523_REG_INTR_DIS(0), 0xff);
	if (ret)
		return ret;
	ret = regmap_write(awi->regmap, AW9523_REG_INTR_DIS(p1_pin), 0xff);
	if (ret)
		return ret;

	/* Clear setup-generated interrupts */
	ret = __aw9523_get_port_state(awi->regmap, 0, 0, &val);
	if (ret)
		return ret;
	ret = __aw9523_get_port_state(awi->regmap,
				      AW9523_PINS_PER_PORT, 0, &val);
	if (ret)
		return ret;
			...
			const struct i2c_device_id *id)
{
	...
	awi->dev = dev;
	awi->reset_gpio = devm_gpiod_get(&client->dev, "reset", GPIOD_OUT_HIGH);
	...
	awi->vio_vreg = devm_regulator_get_optional(&client->dev, "vio");
	...
	ret = aw9523_init_gpiochip(awi, pdesc);
	if (ret)
		goto err_disable_vregs;
